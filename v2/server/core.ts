...
export interface Envelope { protocol: 2; version: number; timestamp: number; nonce: string; iv: string; ciphertext: string; signature: string }
export const associatedData = (e: Pick<Envelope, 'version' | 'timestamp' | 'nonce'>) => `relay-v2\n${e.version}\n${e.timestamp}\n${e.nonce}`;
...
export function validateEnvelope(value: unknown, now: number, windowSeconds: number): Envelope {
  ...
  if (Object.keys(e).sort().join(',') !== 'ciphertext,iv,nonce,protocol,signature,timestamp,version' || e.protocol !== 2 || !Number.isSafeInteger(e.version) || e.version <= 0 || !Number.isSafeInteger(e.timestamp)) throw new RelayError(400, 'invalid_request');
...
export async function makeEnvelope(text: string, encryption: string, signing: string, version = Date.now(), timestamp = Date.now()): Promise<Envelope> {
  ...
  const base = { protocol: 2 as const, version, timestamp, nonce: b64(crypto.getRandomValues(new Uint8Array(24))) };
...
export interface Session { id: string; device: string; exp: number }
export async function issueSession(session: Session, secret: string): Promise<string> {
  const payload = b64(utf8.encode(JSON.stringify(session)));
  return `${payload}.${await hmac(secret, 'session-v2.' + payload)}`;
}
export async function verifySession(token: string, secret: string, now = Date.now()): Promise<Session> {
  const parts = token.split('.');
  if (parts.length !== 2 || token.length > 1024 || !await verifyHmac(secret, 'session-v2.' + parts[0], parts[1])) throw new RelayError(401, 'session_expired');
  try {
    const p = JSON.parse(decoder.decode(unb64(parts[0], 900))) as Session;
    if (typeof p.id !== 'string' || typeof p.device !== 'string' || !Number.isSafeInteger(p.exp) || p.exp <= now) throw new Error();
    return p;
  } catch { throw new RelayError(401, 'session_expired'); }
}
