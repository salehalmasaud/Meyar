...
export default async(req:Request,context:Context)=>{
  ...
  try{
    ...
    const routes=['/status','/exchange','/login','/lock','/config','/state','/upload-ticket','/file-url','/delete-notes','/delete-files'];
    ...
    if(path==='/exchange'&&response.ok){context.cookies.set({name:'__Host-relay_v3',value:data.session,httpOnly:true,secure:true,sameSite:'Strict',path:'/',maxAge:data.max_age});return reply({ok:true});}
