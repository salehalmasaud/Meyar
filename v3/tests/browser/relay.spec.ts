...
import {site,viewer,push,db,png,pdf} from '../helpers.ts';import {hash} from '../../server/core.ts';
...
test('complete inbox and file workflow on live V3 preview',async({browser})=>{
 ...
 const pc=await desktop.newPage(),phone=await mobile.newPage();const owned:string[]=[];
 ...
 try{
  ...
  await pc.locator('.note-copy').click();expect(await pc.evaluate(()=>(window as unknown as {copiedArgument:string}).copiedArgument)).toBe(exact);results.exact_copy_argument=true;
  ...
  const opened=pc.waitForEvent('popup');await pc.getByRole('button',{name:'Open synthetic-image.png',exact:true}).click();const tab=await opened;await expect(tab.locator('img')).toBeVisible();await tab.close();results.file_open=true;
  await pc.getByRole('button',{name:'Link synthetic-image.png',exact:true}).click();await expect(pc.locator('#message')).toContainText('Link copied. It expires in 60 seconds.');results.short_lived_link=true;
  // Retry the actual failed upload queue row after a temporary HTTP failure.
  let fail=true;await desktop.route('**/relay-v3/upload',route=>{if(fail){fail=false;return route.fulfill({status:503,contentType:'application/json',body:'{"error":"temporarily_unavailable"}'});}return route.continue();});
